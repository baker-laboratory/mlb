...
def testfile_of(path, bname, **kw):
    ...
    proj, *path = os.path.split(path)
    path = os.path.join(*path)
    t = f'{proj}/tests/{path}/test_{bname}'
    ic(t)
    if os.path.exists(t):
        return t
...
def main(**kw):
    t = perf_counter()
    if kw['testfile'].endswith(__file__): kw['testfile'] = ''
    cmd, post = dispatch(kw['testfile'], **kw) if kw['testfile'] else (f'{sys.executable} -mpytest', '')
    print("call:", sys.argv)
    print("cwd:", os.getcwd())
    print("cmd:", cmd)
